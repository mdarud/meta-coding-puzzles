...
float getHitProbability(int R, int C, const vector<vector<int>>& G) {
    int totalHits = 0;
    
    // Sum all the values in the 2D vector G
    ...
    return static_cast<float>(totalHits) / (R * C);
}

int main() {
    // Example usage:
    int R = 3, C = 3;
    vector<vector<int>> G = {{1, 0, 1}, {1, 1, 0}, {0, 1, 1}};
    
    cout << "Hit Probability: " << getHitProbability(R, C, G) << endl;
