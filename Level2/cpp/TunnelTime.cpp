...
long long getSecondsElapsed(long long C, int N, vector<long long> A, vector<long long> B, long long K) {
    // Calculate total tunnel time for one full lap
    long long total_tunnel_time = 0;
    for (int i = 0; i < N; ++i) {
        total_tunnel_time += B[i] - A[i];
    }

    // Calculate full laps and remaining tunnel time
    long long laps = K / total_tunnel_time;
    long long remaining_time = K % total_tunnel_time;

    // Total seconds elapsed for full laps
    long long seconds_elapsed = laps * C;

    // If there's no remaining time, return the total time for full laps plus the last tunnel's end time
    if (remaining_time == 0) {
        return seconds_elapsed - C + *max_element(B.begin(), B.end());
    }

    // Sort tunnel start and end times based on start times
    vector<pair<long long, long long>> tunnels;
    for (int i = 0; i < N; ++i) {
        tunnels.push_back({A[i], B[i]});
    }
    sort(tunnels.begin(), tunnels.end());

    long long tunnel_time = 0;
    for (const auto& tunnel : tunnels) {
        long long tunnel_start = tunnel.first;
        long long tunnel_end = tunnel.second;
        long long tunnel_length = tunnel_end - tunnel_start;
...
int main() {
    ...
    long long C = 10;
    int N = 2;
    vector<long long> A = {1, 6}; // Tunnel start times
    vector<long long> B = {3, 7}; // Tunnel end times
    long long K = 5;
    
    cout << "Test Case 1" << endl;
    cout << "Expected Return Value = 13" << endl;  // After 13 seconds, train has spent 5 seconds in tunnels
    cout << "Actual Return Value   = " << getSecondsElapsed(C, N, A, B, K) << endl;
    cout << endl;
    
    // Test Case 2
    C = 100;
    N = 3;
    A = {10, 40, 70}; // Tunnel start times
    B = {20, 50, 80}; // Tunnel end times
    K = 60;
    
    cout << "Test Case 2" << endl;
    cout << "Expected Return Value = 210" << endl;
    cout << "Actual Return Value   = " << getSecondsElapsed(C, N, A, B, K) << endl;
    cout << endl;
    ...
    C = 50;
    N = 1;
    A = {10}; // Tunnel start times
    B = {20}; // Tunnel end times
    K = 100;
    
    cout << "Test Case 3" << endl;
    cout << "Expected Return Value = 520" << endl;  // 10 full laps (500 seconds) plus 20 seconds to reach end of tunnel
    cout << "Actual Return Value   = " << getSecondsElapsed(C, N, A, B, K) << endl;
